...
public class OrderService {
	...
	public Order updateOrder(Order order, Long id) throws OrderNotFoundException, UnauthorizedOrderActionException {
		...
		Order rOrder = findOrder(id);
		
		rOrder.setExpiry(order.getExpiry());
		rOrder.setIsbn(order.getIsbn());
		
		try {
			rOrder = orderRepository.save(rOrder);
		} catch (Exception e) {
			System.out.println("error in updating order with id:" + id);
		}
		return rOrder;		
	}
